...
SELECT ENAME, EXTRACT(YEAR FROM HIREDATE) HIREYEAR, EXTRACT(MONTH FROM HIREDATE) HIREMONTH FROM EMP;
...
SELECT EMPNO, ENAME, HIREDATE, ADD_MONTHS(HIREDATE, 40*12) FROM EMP ORDER BY HIREDATE;

--18. EMP테이블에서 입사일로부터 38년 이상 근무한 직원의 정보 조회
SELECT * FROM EMP WHERE EXTRACT(YEAR FROM SYSDATE)-EXTRACT(YEAR FROM HIREDATE) >= 38;
